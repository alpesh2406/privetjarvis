...
/**
 * Private Jarvis — a personal voice assistant.
 *
 * Flow: Mic button -> Android speech-to-text -> send transcript + conversation
 * history to Claude -> Claude replies AND can emit an [ACTION: ...] tag ->
 * app speaks the reply out loud and performs the action if one was requested.
 *
 * IMPORTANT: Set your Anthropic API key in Config.kt before building.
 */
class MainActivity : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    // Keeps the running chat history so Claude has context across turns
    ...
    /**
     * Calls the Claude API with the conversation so far.
     * Returns the spoken reply, plus an optional action string parsed out of
     * an [ACTION: type|param] tag if Claude decided one is needed.
     */
    ...
    /** Executes a simple action Claude requested, e.g. "alarm|7:00" or "whatsapp|9999999999|Hello" */
